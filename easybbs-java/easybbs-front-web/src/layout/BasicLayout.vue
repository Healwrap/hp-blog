        ...
        <div class="menu">
          ...
          <el-menu class="board-menu" mode="horizontal">
            <template v-for="nav in boardList">
              <el-menu-item v-if="Array.prototype.isPrototypeOf(nav.children) && nav.children.length === 0" :index="String(nav.index)"
                >{{ nav.boardName }}
              </el-menu-item>
              <el-sub-menu v-else :index="String(nav.boardId)">
                ...
                <el-menu-item v-for="item in nav.children" :index="String(item.boardId)">{{ item.boardName }} </el-menu-item>
              </el-sub-menu>
            </template>
          </el-menu>
        </div>
